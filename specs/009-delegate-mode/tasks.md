...
- [ ] T041 [US3] **BLOCKED — Cursor's `agent` executable is not installed on this machine.** Run quickstart Scenario 8 with Cursor Agent installed, and update `research.md` and `contracts/cli-surface.md` with what was actually observed; every Cursor claim is documentation-derived until this task closes (research open item)

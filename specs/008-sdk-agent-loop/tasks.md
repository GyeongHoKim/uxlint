...
- [ ] T018 Final validation per `quickstart.md`: full `npm test` (build + prettier + xo + ava), `npm run test:coverage` exiting nonzero-below-threshold semantics confirmed (SC-007), equivalence re-run (T009 artefacts current), optional live smoke with credentials + Chrome if available. Conventional commit(s) per task group throughout, per husky/commitlint.

...
4. **Given** a container that runs as the root user, **When** a run starts, **Then** analysis proceeds normally rather than the browser terminating on startup, and the output states that the browser's security sandbox was disabled to make that possible
...
- **FR-009**: Runs in privileged container environments MUST complete analysis successfully without requiring the user to discover and pass browser-level workarounds themselves. Where this is achieved by relaxing a browser security protection, the run output MUST state that the protection was relaxed and why.
...
- **SC-004**: A run in a privileged container completes analysis with no user-supplied browser workarounds, and its output names the security protection that was relaxed to allow it.
